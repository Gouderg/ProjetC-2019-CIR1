...
Carte initCarte(const char *nomFichier) {
	...
	*ptr = '\0';
	...
	carteRecup.tabCarte = (char**) malloc(carteRecup.largeur * sizeof(char *));
	for (int i = 0; i < carteRecup.largeur; ++i) {

		carteRecup.tabCarte[i] = (char *) malloc(carteRecup.hauteur * sizeof(int));
	}
